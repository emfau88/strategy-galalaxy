...
The portrait backgrounds, all structures outside `assets/library/galalaxy/`, both generations of the eight hulls under `assets/factions/unified/`, the two engine strips under `assets/effects/`, the eight projectile bodies under `assets/projectiles/unified/` and the command medallion were generated specifically for Strategy Galalaxy and are not Galalaxy reference assets. Active generated structures include the Orbital Garden HQ pair, team-integrated modular Garden turrets and unified energy relay. Their prompts and runtime adaptations are recorded in `docs/GENERATED_ASSETS.md`; common rendering rules are recorded in `docs/ART_DIRECTION.md`.
