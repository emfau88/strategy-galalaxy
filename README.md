...
The Canvas foundation, headless two-lane combat, persistent Command-/Battle-cycle, Economy core, and the first visual slice are complete. The current build establishes mobile portrait scaling, deterministic combat, free base waves, Energy, capturable Nodes, upgrades, CC0 fleet art, projectile art, hit feedback, and a rule-bound opponent planner. The player UI follows in the next bulk.
...
In the normal build, a tap starts a Command Phase; a tap during Command deploys the currently free base waves. `P` pauses or resumes an active phase. `?test=match` enters the automatic repeatable test match immediately.
